var Snake = (function() {
  ...
  var root_2$1 = /* @__PURE__ */ from_html(`<li class="svelte-mt0erd"><!></li>`);
  var root$1 = /* @__PURE__ */ from_html(`<ol class="svelte-mt0erd"><!> <!></ol>`);
  function Grid2D$1($$anchor, $$props) {
    ...
    each(node_1, 17, () => $$props.grid.array, index, ($$anchor2, row, r) => {
      var fragment = comment();
      var node_2 = first_child(fragment);
      each(node_2, 17, () => get(row), index, ($$anchor3, _, c) => {
        var li = root_2$1();
        var node_3 = child(li);
        snippet(node_3, () => $$props.cell ?? noop, () => c, () => r);
        append($$anchor3, li);
      });
  ...
  class Snake2 {
    ...
    try(moveFn, strategyFn) {
      const head2 = this.vectors.at(-1);
      if (!head2) return this;
      const newHead = moveFn(head2);
      if (!newHead || this.hasCollision(newHead)) return this;
      const newVectors = strategyFn(this.vectors, newHead);
      return new Snake2(newVectors);
    }
    isNeck(v) {
      const neck = this.vectors.at(-2);
      if (neck?.x === v.x && neck?.y === v.y) return true;
      return false;
    }
    hasCollision(v) {
      return this.vectors.some((self) => v.x === self.x && v.y === self.y);
    }
    static eat(vectors, newHead) {
      return [...vectors, newHead];
    }
    static move(vectors, newHead) {
      return [...vectors.slice(1), newHead];
    }
  }
  var root_1 = /* @__PURE__ */ from_html(`<link rel="stylesheet" href="/style.css"/>`);
  var root_2 = /* @__PURE__ */ from_html(`<div></div>`);
  var root = /* @__PURE__ */ from_html(`<!> <header> </header> <!>`, 1);
  function App($$anchor, $$props) {
    push($$props, true);
    const grid = new Grid2D(20, 20);
    const move2 = grid.transformer;
    const isFood = (v) => v.x === get(food).x && v.y === get(food).y;
    const newFood = () => {
      const v = new Vector(getRandomInt(0, grid.width), getRandomInt(0, grid.height));
      if (get(snake).vectors.some((s) => s.x === v.x && s.y === v.y)) return newFood();
      return v;
    };
    function changeDirection(nextDirection) {
      const v = move2[nextDirection](get(snake).vectors.at(-1));
      if (v === null) return set(direction, nextDirection, true);
      if (get(snake).isNeck(v)) return console.log("That's ma neck");
      set(direction, nextDirection, true);
    }
    function updateSnake() {
      const vNext = Vector.add(grid[get(direction).toUpperCase()])(get(snake).vectors.at(-1));
      const strategyFn = isFood(vNext) ? Snake2.eat : Snake2.move;
      if (isFood(vNext)) set(food, newFood());
      const newSnake = get(snake).try(move2[get(direction)], strategyFn);
      if (newSnake === get(snake)) return console.log("Game over");
      set(snake, newSnake, true);
    }
    let snake = /* @__PURE__ */ state(proxy(new Snake2([new Vector(9, 10), new Vector(10, 10)])));
    let score = /* @__PURE__ */ user_derived(() => get(snake).vectors.length);
    let direction = /* @__PURE__ */ state("right");
    let food = /* @__PURE__ */ state(newFood());
    let counter = /* @__PURE__ */ state(0);
    const threshold = 0.1;
    function update(delta) {
      set(counter, get(counter) + delta);
      if (get(counter) < threshold) return;
      set(counter, 0);
      updateSnake();
    }
    ...
    JoinLoop(node, { update });
    var header = sibling(node, 2);
    var text = child(header);
    var node_1 = sibling(header, 2);
    {
      const cell = ($$anchor2, x = noop, y = noop) => {
        var div = root_2();
        let classes;
        template_effect(($0) => classes = set_class(div, 1, "svelte-1n46o8q", null, classes, $0), [
          () => ({
            active: get(snake).vectors.find((v) => v.x === x() && v.y === y()),
            food: isFood(new Vector(x(), y()))
          })
        ]);
        append($$anchor2, div);
      };
      component(node_1, () => grid.component, ($$anchor2, grid_component) => {
        grid_component($$anchor2, { cell, $$slots: { cell: true } });
      });
    }
    template_effect(() => set_text(text, `Score: ${get(score) ?? ""}`));
